...
module.exports = ({
  ...
  app.get("/protocol-version", (req, res) => {
    ...
    res.json({ version: 9 });
    console.log(`/protocol-version Returning`);
  });
  ...
  app.get("/ice-servers", (req, res) => {
    ...
    res.status(200).json([]); // MUST send array back to work currently with Teletype Client
    console.log(`/ice-servers Returning`);
  });
  ...
  app.post("/peers/:id/signals", async (req, res) => {
    ...
    res.send({});
    console.log(`/peers/:id/signals Returning: senderId: ${senderId} - signal: ${signal} - sequenceNumber: ${sequenceNumber} - testEpoch: ${testEpoch}`);
  });
  ...
  app.post("/portals", async (req, res) => {
    ...
    console.log(`/portals Returning: id: ${id}`);
  });
  ...
  app.get("/portals/:id", async (req, res) => {
    ...
    if (portal) {
      ...
      console.log(`/portals/:id Returning HAPPY: portal: ${portal} - hostPeerId: ${portal.hostPeerId}`);
    } else {
      res.status(404).json({});
      console.log(`/portals/:id Returning SAD:`);
    }
  });
  ...
  app.get("/_ping", async (req, res) => {
    ...
    if (!statuses[0]) {
      unhealthyServices.push("pubSubGateway");
    }
    if (!statuses[1]) {
